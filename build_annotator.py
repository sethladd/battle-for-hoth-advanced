...
function render(){
 ...
 for(let c=0;c<GEO.cols;c++)for(let r=0;r<GEO.rows;r++){
   ...
   if(cell.mark){const it=PALETTE.find(p=>p.k==cell.mark);
     const m=document.createElementNS(NS,'rect');m.setAttribute('x',x-10);m.setAttribute('y',y-10);
     m.setAttribute('width',20);m.setAttribute('height',20);m.setAttribute('fill','none');
     m.setAttribute('stroke',it.c);m.setAttribute('stroke-width','3');m.style.pointerEvents='none';ov.appendChild(m);}
...
function convertIn(txt){const d=JSON.parse(txt);const st={};
 ...
 for(const s in d){if(s==='_geo')continue;st[s]={};const o=d[s];
